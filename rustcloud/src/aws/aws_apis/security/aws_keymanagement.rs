...
use aws_sdk_kms::{
    types::{KeySpec, KeyUsageType, OriginType, Tag},
    Client, Error,
};
...
pub async fn create_key(
    ...
    let resp = client
        ...
        .set_custom_key_store_id(custom_key_store_id)
        .set_bypass_policy_lockout_safety_check(bypass_policy_lockout_safety_check)
        .set_tags(tags)
        .set_multi_region(multi_region)
        .set_xks_key_id(xks_key_id)
        .send()
        .await;
    match resp {
        Ok(result) => {
