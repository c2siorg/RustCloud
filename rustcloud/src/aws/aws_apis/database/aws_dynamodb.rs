...
use tokio;
use aws_sdk_dynamodb::{types::{AttributeDefinition, AttributeValue, BillingMode, Condition, ConditionalOperator, ExpectedAttributeValue, GlobalSecondaryIndex, KeySchemaElement, LocalSecondaryIndex, OnDemandThroughput, ProvisionedThroughput, ReturnConsumedCapacity, ReturnItemCollectionMetrics, ReturnValue, ReturnValuesOnConditionCheckFailure, Select, SseSpecification, StreamSpecification, TableClass, Tag}, Client, Error};


pub async fn create_table(client : &Client, attribute_definitions:AttributeDefinition,table_name: String,key_schema:KeySchemaElement,local_secondary_indexes: Option<Vec<LocalSecondaryIndex>>,global_secondary_index: Option<Vec<GlobalSecondaryIndex>>,billing_mode: BillingMode,provisioned_throughput :ProvisionedThroughput,stream_specification :Option<StreamSpecification>,sse_specification :Option<SseSpecification>,tags :Option<Vec<Tag>>,
 table_class: TableClass,deletion_protection_enabled: Option<bool>,resource_policy: Option<String>,on_demand_throughput: Option<OnDemandThroughput>) -> Result<(), Error> {
    let resp = client.create_table().attribute_definitions(attribute_definitions).table_name(table_name).key_schema(key_schema).set_local_secondary_indexes(local_secondary_indexes).set_global_secondary_indexes(global_secondary_index).billing_mode(billing_mode).provisioned_throughput(provisioned_throughput).set_stream_specification(stream_specification).set_sse_specification(sse_specification).set_tags(tags).table_class(table_class).set_deletion_protection_enabled(deletion_protection_enabled).set_resource_policy(resource_policy).set_on_demand_throughput(on_demand_throughput).send().await;
