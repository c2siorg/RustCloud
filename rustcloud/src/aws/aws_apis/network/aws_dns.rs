...
use aws_sdk_route53::{
    types::{ChangeBatch, HostedZoneConfig, HostedZoneType, Vpc},
    Client, Error,
};
...
pub async fn create_zone(
    ...
    let resp = client
        ...
        .set_hosted_zone_config(hosted_zone_config)
        .set_delegation_set_id(delegation_set_id)
        .send()
        .await;
...
pub async fn list_zones(
    ...
    delegation_set_id: Option<String>,
    hosted_zone_type: Option<HostedZoneType>,
) -> Result<(), Error> {
    let resp = client
        .list_hosted_zones()
        .set_marker(marker)
        .set_max_items(max_items)
        .set_hosted_zone_type(hosted_zone_type)
        .set_delegation_set_id(delegation_set_id)
        .send()
        .await;
    match resp {
