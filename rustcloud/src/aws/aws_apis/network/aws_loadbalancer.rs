...
pub async fn add_tags(client: &Client, load_balancer_name: String, tags: Tag) -> Result<(), Error> {
    let resp = client.add_tags().load_balancer_names(load_balancer_name).tags(tags).send().await;
    match resp {
        Ok(result) =>{        
            println!("{:?}", result);
            Ok(())
...
pub async fn create(client: &Client, load_balancer_name: String,listeners: Option<Vec<Listener>>,availability_zones: Option<Vec<String>>,subnets: Option<Vec<String>>,security_groups: Option<Vec<String>>,scheme: Option<String>,tags: Option<Vec<Tag>>) -> Result<(), Error> {
    let resp = client.create_load_balancer().load_balancer_name(load_balancer_name).set_listeners(listeners).set_availability_zones(availability_zones).set_security_groups(security_groups).set_scheme(scheme).set_tags(tags).set_subnets(subnets).send().await;
    match resp {
        Ok(result) =>{        
            println!("{:?}", result);
            Ok(())
...
pub async fn delete(client: &Client, load_balancer_name: String) -> Result<(), Error> {
    let resp = client.delete_load_balancer().load_balancer_name(load_balancer_name).send().await;
    match resp {
        Ok(result) =>{        
            println!("{:?}", result);
            Ok(())
...
pub async fn describe(client: &Client, load_balancer_name: String) -> Result<(), Error> {
    let resp = client.describe_load_balancer_attributes().load_balancer_name(load_balancer_name).send().await;
    match resp {
        Ok(result) =>{        
            println!("{:?}", result);
            Ok(())
