...
async fn test_create_dataset() {
    ...
    client.delete_dataset("your_dataset_id").await.ok();
    let result = client.create_dataset("your_dataset_id").await;
    assert!(result.is_ok());
...
async fn test_delete_dataset() {
    let client = create_client().await;
    let result = client.delete_dataset("your_dataset_id").await;
    assert!(result.is_ok());
    let response = result.unwrap();
...
async fn test_create_table() {
    ...
    let result = client
        .create_table("your_dataset_id", "your_table_id", fields)
        .await;
    assert!(result.is_ok());
    let response = result.unwrap();
    assert_eq!(response["status"], 200);
...
async fn test_delete_table() {
    ...
    client.delete_dataset("test_delete_table_ds").await.ok();
    assert!(result.is_ok());
    let response = result.unwrap();
    assert_eq!(response["status"], 204);
}
...
async fn test_list_tables() {
    ...
    client.delete_dataset("test_list_tables_ds").await.ok();
    assert!(result.is_ok());
    let response = result.unwrap();
    assert_eq!(response["status"], 200);
}
