use crate::types::llm::{LlmRequest, Message, ModelRef, FinishReason, UsageStats, LlmResponse};
...
fn test_llm_request_builder() {
    let request = LlmRequest {
        ...
        messages: vec![
            ...
            Message { role: "model".to_string(), content: "Hi there".to_string() },
        ],
        max_tokens: Some(100),
...
fn test_message_role_validation() {
    let user_msg = Message { role: "user".to_string(), content: "Test".to_string() };
    let model_msg = Message { role: "model".to_string(), content: "Response".to_string() };
