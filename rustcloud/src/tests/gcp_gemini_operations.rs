...
use crate::gcp::gcp_apis::artificial_intelligence::gcp_gemini::GeminiAdapter;
#[allow(unused_imports)]
...
fn create_adapter() -> GeminiAdapter {
    let _ = rustls::crypto::ring::default_provider().install_default();
    let api_key = std::env::var("GEMINI_API_KEY").unwrap_or_default();
    GeminiAdapter::new(api_key, "gemini-2.5-flash".to_string())
}
...
async fn test_gemini_embed() {
    ...
    if let Err(ref e) = result { println!("EMBED ERROR: {}", e); }
    assert!(result.is_ok());
